...
POWER_RESP = 1
cardiac = False
FREQ_CARD_BPM = 0                   # phantom period cardiac [s]
AMP_PHANTOM_CARD_PP = 0                     # phantom amplitude [mm]
TRACE_FUNC_CARD = np.cos                  #reference motion function
POWER_CARD = 0
...
if cardiac:
    if POWER_CARD >1:
        FREQ_CARD_BPM = FREQ_CARD_BPM / 2 # count peaks 
        A_card = AMP_PHANTOM_CARD_PP
    else:
        FREQ_CARD_BPM = FREQ_CARD_BPM
        A_card = AMP_PHANTOM_CARD_PP / 2
else:
    A_card = 0.0

if POWER_RESP >1:
    FREQ_RESP_CPM = FREQ_RESP_CPM / 2 # count peaks
    A_resp = AMP_PHANTOM_RESP_PP
else:
    A_resp = AMP_PHANTOM_RESP_PP / 2



omega_resp = 2*np.pi*(FREQ_RESP_CPM/60)
omega_card = 2*np.pi*(FREQ_CARD_BPM/60) if cardiac else None

# ---- start fitting only after threshold ----
fit_start_idx = np.argmax(mean_y > START_ERROR_CALC_VALUE_Y)
if mean_y[fit_start_idx] <= START_ERROR_CALC_VALUE_Y:
    raise RuntimeError("No samples above START_ERROR_CALC_VALUE_Y for fitting")

fit_mask_y = np.zeros_like(mean_y, dtype=bool)
fit_mask_y[fit_start_idx:] = True
...
# ---- phantom trace ----
def phantom_trace(t):
    resp = A_resp * TRACE_FUNC_RESP(omega_resp * t) ** POWER_RESP
    if cardiac:
        card = A_card * (TRACE_FUNC_CARD(omega_card * t) ** POWER_CARD)
        trace = resp + card
    else:
        trace = resp
    trace -= np.mean(trace)
...
report.add("Signal Synchronization", [
    ...
    ("Respiratory frequency", (FREQ_RESP_CPM/60, " Hz")),
    ("Cardiac frequency", (FREQ_CARD_BPM/60 if cardiac else 0, " Hz")),
])
...
start_idx = np.argmax(mean_y > START_ERROR_CALC_VALUE_Y)
if mean_y[start_idx] <= START_ERROR_CALC_VALUE_Y:
    print("No valid data → skipping evaluation")
    valid_eval_mask = np.zeros_like(mean_y, dtype=bool)
else:
    valid_eval_mask = np.zeros_like(mean_y, dtype=bool)
    valid_eval_mask[start_idx:] = True
...
start_idx_fine = np.argmax(mean_y_fine > START_ERROR_CALC_VALUE_Y)
if mean_y_fine[start_idx_fine] <= START_ERROR_CALC_VALUE_Y:
    valid_eval_mask_fine = np.zeros_like(mean_y_fine, dtype=bool)
else:
    valid_eval_mask_fine = np.zeros_like(mean_y_fine, dtype=bool)
    valid_eval_mask_fine[start_idx_fine:] = True
...
plot_mask = np.zeros_like(time_s, dtype=bool)
start_idx = np.argmax(mean_y > START_ERROR_CALC_VALUE_Y)
if mean_y[start_idx] > START_ERROR_CALC_VALUE_Y:
    plot_mask[start_idx:] = True

plot_mask_fine = np.zeros_like(t_fine, dtype=bool)
start_idx_fine = np.argmax(mean_y_fine > START_ERROR_CALC_VALUE_Y)
if mean_y_fine[start_idx_fine] > START_ERROR_CALC_VALUE_Y:
    plot_mask_fine[start_idx_fine:] = True
...
# Errors 
valid_idx = np.argmax(mean_y > START_ERROR_CALC_VALUE_Y)
if mean_y[valid_idx] <= START_ERROR_CALC_VALUE_Y:
    print("No data above threshold → skipping error plotting")
else:
    if len(angles) > 0:
        prev_angle = angles[0]
        for i in range(1, len(angles)):
            if angles[i] != prev_angle:
                ...
                if 0 <= idx < len(time_s):
                    t0 = time_s[idx]
                    t1 = t0 + EXCLUDE_AFTER_ANGLE_CHANGE_S
                    for ax in (ax_ex, ax_ey):
                        ax.axvspan(t0, t1, color="gray", alpha=0.25, zorder=0)
                prev_angle = angles[i]
    ...
    valid_idx_fine = np.argmax(mean_y_fine > START_ERROR_CALC_VALUE_Y)
    if mean_y_fine[valid_idx_fine] > START_ERROR_CALC_VALUE_Y:
        # Interpolated errors
...
if len(angles) and len(samples) > 0:
    ...
    if first_angle_pos < len(angles):
        ...
        for i in range(first_angle_pos + 1, len(angles)):
            ...
            idx = angle_idx[i]
            if angle != prev_angle:
                for ax in [ax_x, ax_y, ax_ex, ax_ey]:
...
# Layout & save 
fig.suptitle(f"Motion Estimation vs Reference — {os.path.basename(latest_log)}",y=0.97)
