#%%
print('START MASTER PREDICTION MODULE')
import os
import sys
import threading
import asyncio
...
parser = argparse.ArgumentParser(description='Master Prediction Module')
...
class predictor:
    ...
    def __init__(self,receive_timestamps=False,send_frequency=None):
        ...
        LOG_DIR_LSTM = os.path.join(config["logging"]["folder"],args.t_logging, "lstm")
        ...
        if config['logging']['lstm_pred_log']:
            self.log_file_path_pred = os.path.join(LOG_DIR_LSTM, f"pred_log_file_{ts}.txt")
        ...
        if config['logging']['lstm_optimization_log']:
            self.log_file_path_optimization = os.path.join(LOG_DIR_LSTM, f"online_lstm_opt_log_{ts}.txt")
    ...
    def connect(self,host, port): # receiver
        ...
        if self.connect_to_external_motion_estimation:
            ...
            print(f"Prediction module connecting to external motion estimation at {host}:{port}...")
            self.conn = self.socket
        else:
            print(f"Prediction module connecting to SAM at {host}:{port}...")
    ...
    def connect_to_gui(self, host=host_gui, port=port_gui): # sender
        if config['settings']['enable_gui']:
            ...
            print("GUI is disabled in config. Not connecting to GUI.")
    ...
    def connect_sender(self,host=host_send, port=port_send): # sender
        if self.send_frequency is not None:
            ...
            self.socket.bind(f"tcp://{host}:{port}")
            print(f"Prediction module waiting for ZMQ connection on {host}:{port}...")
