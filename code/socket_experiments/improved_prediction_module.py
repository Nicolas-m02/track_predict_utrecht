...
host = "0.0.0.0"
port = 9000  # change to 1221 for COM from SAM2
testing = False
...
class predictor:
    ...
    def __init__(self):
        ...
        # Async queues for better concurrency
        self.in_data_queue = asyncio.Queue()
    ...
    async def predict(self):
        while True:
            ...
            data = await self.in_data_queue.get()
            ...
            # prediction = await self.make_prediction(data)
            print('----')
            print(f"Current data point: {self.current_data_point}")
            print(f"Current prediction point: {self.current_prediction_point}")
            ...
            await asyncio.sleep(0.005)  # Sleep briefly to yield control to other tasks
    ...
    async def receive_data(self):
        ...
        while True:
            ...
            data = await loop.sock_recv(self.conn, 1024)

        
            await self.in_data_queue.put(torch.tensor(struct.unpack('2f', data)).float().to(device))
        ...
        if testing:
            ...
            if len(self.seen_data) >= self.input_size:
                ...
                with torch.no_grad():
                    ...
                    await self.prediction_queue.put(output)
...
async def main():
    ...
    await asyncio.gather(
                                ...
                                #prediction_instance.close_connection()
                                )
